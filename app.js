        ...
        jwt.verify(req.token,'pe-tests-admin',(err,authData)=>{
            ...
            else
            {
                ...
                req.body.question_images_white.forEach(image=>{
                    question_images_white.push(Buffer.from(atob(atob(image)),"base64"));
                })

                req.body.question_images_black.forEach(image=>{
                    question_images_black.push(Buffer.from(atob(atob(image)),"base64"));
                })

                req.body.answer_images_white.forEach(image=>{
                    answer_images_white.push(Buffer.from(atob(atob(image)),"base64"));
                })

                req.body.answer_images_black.forEach(image=>{
                    answer_images_black.push(Buffer.from(atob(atob(image)),"base64"));
                })
